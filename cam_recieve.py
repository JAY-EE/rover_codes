#!/usr/bin/env python3
"""
ROS 2 GStreamer RECEIVER Launch Script.

This script runs on the Ground Station / Main PC.
It uses the ROS 2 launch API to start 6 gscam_node instances.
Each node listens on a unique UDP port for an H264 stream,
decodes it, and publishes it to a unique ROS 2 topic.
"""
...
#
# This MUST match the config on your robot_sender.py
#
camera_configs = [
...
def main():
    # Base GStreamer pipeline string for the RECEIVER.
    ...
    base_pipeline = (
        ...
        # NOTE: If your GCS has an NVIDIA GPU, you can get better performance
        # by replacing 'avdec_h264' with 'nvdec_h264' (or 'nvmpi' on Jetson).
        # 'avdec_h264' is a software decoder and will always work, but uses CPU.
    )

    ld = LaunchDescription()

    print(f"Preparing to launch {len(camera_configs)} camera receiver nodes...")
    ...
    for config in camera_configs:
        ...
        gscam_node = Node(
            ...
            # Use remappings to set the output topics
            remappings=[
        ...
        print(f"  - Adding receiver '{cam_name}' (Port: {cam_port}) -> Topic: /cameras/{target_image_topic}")
